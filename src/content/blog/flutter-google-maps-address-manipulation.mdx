...
In the [Second post](/posts/flutter-google-maps-embedded-map/) we have implemented a simple Flutter app with an embedded Google Map.
...
We will use `http` package introduced in the [First post](/posts/flutter-google-maps-setup/) to call Google Maps API.
