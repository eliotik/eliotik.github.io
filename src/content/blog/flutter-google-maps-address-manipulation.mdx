...
tags:
    ...
    - google maps
    - address
description: How to convert coordinates to address and vice versa in Flutter Google Maps
---
...
```dart title='src/screens/home.dart' collapse={1-82,94-120} showLineNumbers=false {"Replace 'LatLng' with our UserLocation interface":85-86}
...
```dart title='src/screens/home.dart' collapse={3-5,8-9,12-22,32-38,40-89,118-126,154-183} showLineNumbers=true {"This package we need to parse JSON":1-2} {"dotenv package to read API key from environment variables":6-7} {"http package to send network requests":10-11} {"Here we read in advance API key from env variable":25-26} {"Kind of state variables: show loading animation or UNKNOWN value if adress fetch failed":29-31} {"Here we enable address loading animation":93-94} {"And here we use our new helper method to fetch address based on coordinates":97-98} {"Core change: helper method to send GET request to Google Maps API and parse response":101-115} {"When we close Map screen we also want to update address value":130-135} {"Now we will show loading animation when waiting on API response":140-141} {"If address fetching's done, we will replace animation":143-144} {"And as a value we will use fetched address from API":146-147}
//
import 'dart:convert';
