...
- **Astro config**: `astro.config.mjs` - Integrations include MDX, React, Partytown (for analytics), and Expressive Code (for syntax highlighting with line numbers and collapsible sections); Tailwind 4 via the `@tailwindcss/vite` Vite plugin (it is no longer an Astro integration). Markdown uses the unified processor (`@astrojs/markdown-remark`) so remark-toc and remark-collapse keep working under Astro 7. Sitemaps are generated by custom endpoints in `src/pages/sitemap-*.xml.ts` (see `src/utils/sitemapXml.ts`).
...
- Node version: 22.23.3 (specified in `.nvmrc` and `package.json` engines)
