...
## 7. Addendum: Phase 2 (2026-09-24)

### 7.1 Requests

User direction on 2026-09-24 asked for:
1. Fix the Partytown/ClientRouter console error.
2. Update dev tools (audit-flagged/deprecated transitive packages, pnpm).
3. Decide whether to enforce Prettier formatting on `.astro` files.
4. Decide whether to keep or remove `@astrojs/sitemap`.
5. Clean up the CI and Dependabot configuration.

### 7.2 Decisions

- **`.astro` formatting**: enforce Prettier on `.astro` files via non-overlapping `lint-staged` globs plus a CI `pnpm format:check` step.
- **`@astrojs/sitemap`**: removed — sitemaps are generated by the custom `src/pages/sitemap-*.xml.ts` endpoints, and the dependency had been unused.
- **CI/Dependabot cleanups**: pnpm is resolved from `packageManager` in CI instead of a floating `corepack prepare pnpm@latest`; Dependabot grouping and `exclude-patterns` were tightened; and, after the Phase 2 review, an `ignore:` cap was added for `@types/node` majors and to hold `typescript` below 6.1.
- **Dev-tooling refresh**: a lockfile-only refresh of transitive dependencies, within their declared ranges, for audit-flagged/deprecated packages; `packageManager` bumped to pnpm 10.34.5; `@types/node` added as an explicit devDependency pinned exactly to 22.x.
- **Astro/tooling bumps**: astro 7.3.4 → 7.3.5, astro-eslint-parser 3.1.0 → 3.2.0 (brings `@astrojs/compiler-rs` 0.5.0).
- **Partytown fix**: `sandboxParent: 'html'` (astro.config.mjs) plus `transition:persist` on both gtag scripts (src/layouts/Layout.astro), chosen after a root-cause investigation. Root cause: `@astrojs/partytown`'s `astro:before-swap` hook moved the sandbox iframe into the incoming document on every ClientRouter navigation, destroying the sandbox and its worker (aborting in-flight `/~partytown/proxytown` XHRs) and restarting gtag. Keeping the sandbox on `<html>`, outside `<body>`, means it is never part of the subtree Astro swaps, so it survives for the whole visit; `transition:persist` on the gtag scripts keeps their already-executed state across the swap instead of re-running them. The fix depends on the GA4 "Page changes based on browser history events" setting staying on, so that GA4 records the soft navigations.
- **LinkButton workaround removed**: the scope-class-forwarding workaround added during the Astro 7 migration was removed once `@astrojs/compiler-rs` 0.5.0 fixed the underlying class-forwarding bug upstream.

### 7.3 Phase 2 commits

Commits in `58f9565..35d1250`, oldest first; the commit that adds this addendum and the Dependabot caps follows them.

- `d5a4218` build(T-39): remove unused @astrojs/sitemap
- `5179209` build(T-39): enforce Prettier on .astro files (lint-staged + CI)
- `98d72fd` ci(T-39): pnpm from packageManager in CI, Dependabot grouping, prune onlyBuiltDependencies
- `b80d41b` ci(T-39): note that Dependabot exclude-patterns mirror the named groups
- `cfad89f` build(T-39): refresh dev tooling transitive deps, pnpm 10.x latest
- `4d323d5` build(T-39): bump astro 7.3.5, astro-eslint-parser 3.2.0
- `80a9626` fix(T-39): keep the Partytown sandbox alive across ClientRouter navigations
- `66dcef2` refactor(T-39): drop LinkButton scope-class forwarding (fixed in compiler-rs 0.5)
- `35d1250` docs(T-39): Task 9 PR title and interim verification-report status after Phase 2 review
